...
export default function Messages() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl border border-line bg-bg divide-y divide-line overflow-hidden">
        {items === null && (
        ...
        {items?.map(m => (
          <div key={m.id}>
            <button
              onClick={() => open(m)}
              className={`w-full text-left px-5 py-4 flex items-center gap-4 hover:bg-surface/40 transition ${
                !m.is_read ? 'bg-surface/30' : ''
              }`}
            >
              <span
                className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                  m.is_read ? 'bg-muted/40' : 'bg-good'
                }`}
              />
                ...
                <div className="flex items-baseline gap-2">
                  <span
                    className={`text-sm truncate ${
                      m.is_read ? 'text-muted' : 'text-fg font-medium'
                    }`}
                  >
                    {m.name || m.email}
                  </span>
              ...
              <span className="text-[10px] text-muted font-mono uppercase tracking-wider flex-shrink-0">
                /{m.source}
              </span>
              ...
              {openId === m.id && (
                ...
                >
                  <div className="px-5 py-4 grid grid-cols-[max-content_1fr] gap-x-6 gap-y-2 text-sm">
                    ...
                    {m.message && (
                      <KV k="сообщение">
                        <p className="whitespace-pre-wrap text-fg">{m.message}</p>
                      </KV>
                    )}
...
function KV({ k, children }: { k: string; children: React.ReactNode }) {
  return (
    <>
      <span className="text-xs uppercase tracking-wider text-muted pt-0.5">
        {k}
      </span>
      <span>{children}</span>
    </>
